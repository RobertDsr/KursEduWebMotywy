...
function the_post_breadcrumb() {
    ...
    if(count($recipe_cats) > 0 ) {
        ...
        $recope_meal_type = array_shift(array_values($recipes_cats));
        $term_link = get_term_link( $recope_meal_type->slug, 'meal-type' );

        echo '<a href="'.$term_link.'">'.ucfirst($post_type_name).'</a>'
